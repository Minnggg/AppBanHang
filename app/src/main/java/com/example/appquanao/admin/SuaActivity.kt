...
import com.example.appquanao.databinding.ActivitySuaBinding
...
class SuaActivity : AppCompatActivity() {
    private lateinit var binding: ActivitySuaBinding
    private var storage =  Firebase.storage
    private lateinit var uri: Uri
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = DataBindingUtil.setContentView(this,R.layout.activity_sua)
        ...
        binding.btnThem.setOnClickListener(View.OnClickListener {
            ...
            if(binding.type.text.toString().length!=0
                ...
                val temp =  database.getReference("product/"+binding.type.text.toString()).get().addOnSuccessListener {
                    ...
                    them(binding.id.text.toString().trim().toInt())
                    Toast.makeText(this,"Sửa sản phẩm thành công", Toast.LENGTH_LONG).show()
